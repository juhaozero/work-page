...
`Project` 主要字段：`id`、`slug`、`name`、`category`、`description`、`longDescription?`、`kind?`（`web` | `package`，默认 `web`）、`url?`（web 必填）、`downloadUrl?`（package 必填）、`version?`、`featured?`、`repo?`、`tech?`、`createdAt?`、`lifecycle?`、`demo?`、`docs?`、`changelog?`、`cover?`。
...
```json
{
  ...
  "i18n": {
    "zh": {
      ...
      "longDescription": "没有在线网页，主按钮直达最新安装包。"
    },
    "en": {
      "name": "My App",
      "category": "Tools",
      "description": "Desktop installer distribution.",
      "longDescription": "No web app — primary action downloads the latest installer."
    }
  }
}
```
